# Stable Audio 2 VAE latent baseline evaluation

本文档说明如何在已经预处理好的 `MTG-Jamendo-1000-24k-mono-5s` test 集上，评测 Stable Audio 2 的 VAE/pretransform latent autoencoder baseline，并导出和 `sa3_same_l_listen5` 完全一致的主观听感 sample。

当前项目命令名为 `ae-evaluate-stable-audio-vae`。默认使用：

- `--pretrained-name stabilityai/stable-audio-2`
- `--system-name stable-audio-2-vae-latent`
- `--output-dir outputs/evaluation/stable_audio_2_vae_latent`
...
如果 Stable Audio 2 权重托管在 gated/private Hugging Face repo，需要先登录：
...
```powershell
hf auth whoami
hf download stabilityai/stable-audio-2 model_config.json
```
...
```powershell
--pretrained-name path/to/stable-audio-2
```
...
conda run -n tokenizer-eval python -m ae_research.cli.evaluate_stable_audio_vae `
  ...
  --output-dir outputs/evaluation/stable_audio_2_vae_latent_listen5 `
  --device cuda `
  --batch-size 1 `
  --num-workers 0
```
...
```text
outputs/evaluation/stable_audio_2_vae_latent_listen5/sample_manifest/test.jsonl
```
...
```text
outputs/evaluation/stable_audio_2_vae_latent/
  metrics.json
  mushra_command.txt
  reference/
  stable-audio-2-vae-latent/
```

listen5 评测输出到：

```text
outputs/evaluation/stable_audio_2_vae_latent_listen5/
  metrics.json
  mushra_command.txt
  sample_manifest/
    test.jsonl
...
```powershell
fadtk vggish outputs/evaluation/stable_audio_2_vae_latent/reference outputs/evaluation/stable_audio_2_vae_latent/stable-audio-2-vae-latent
```
...
`mushra_command.txt` 会写入只包含 Stable Audio 2 VAE baseline 的 MUSHRA 准备命令。若要和 SA3 SAME-L 或本项目 autoencoder 一起比较，可以手动加入额外 system：

```powershell
ae-mushra prepare `
  --reference-dir outputs/evaluation/stable_audio_2_vae_latent_listen5/reference `
  --system stable-audio-2-vae-latent=outputs/evaluation/stable_audio_2_vae_latent_listen5/stable-audio-2-vae-latent `
  --system same-l=outputs/evaluation/sa3_same_l_listen5/same-l `
  --system ours=outputs/evaluation/reconstruction `
  --output-dir outputs/mushra_sa2_vae_vs_sa3_vs_ours `
  --sample-rate 24000 `
  --max-trials 5
```

生成后，将 `outputs/mushra_sa2_vae_vs_sa3_vs_ours/manifest.json` 和 `stimuli/` 发给听评者。组织者保留 `key.json`，收集评分后汇总：

```powershell
ae-mushra summarize --scores outputs/mushra_sa2_vae_vs_sa3_vs_ours/scores.csv
```

## 9. 注意事项

- Stable Audio 2 VAE/pretransform 原生采样率和声道数由 `model_config.json` 决定；评测脚本会把输入从 `24 kHz mono` 转到模型格式，decode 后再转回 `24 kHz mono 5s` 计算指标。
- 如果 `stabilityai/stable-audio-2` 报 404，不一定是代码问题，可能是 repo 名称不可见、private/gated 权限不足，或权重尚未缓存。
