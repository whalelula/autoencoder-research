...
def _run_rfad(
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"fadtk {model_name} failed with exit code {result.returncode}; "
            f"see {output_dir / f'rfad_{model_name}.txt'}"
        )
    candidates = []
    ...
    *,
    ...
    if bool(eval_config["export_audio"]):
        ...
        for generated_dir in (reference_dir, reconstruction_dir):
            for stale_file in generated_dir.glob("*.wav"):
                stale_file.unlink()
    if run_rfad and not bool(eval_config["export_audio"]):
    ...
    if run_rfad and max_audio_samples is not None:
        rfad_temp_dir = tempfile.TemporaryDirectory(
            prefix="rfad_audio_", dir=str(output_dir)
        )
    ...
    try:
        ...
        for batch_index, batch in enumerate(tqdm(loader, desc="test evaluation")):
            ...
            sample_rate = int(data_config["sample_rate"])
            for index, raw_track_id in enumerate(batch["track_id"]):
        ...
        if run_rfad:
            summary["rFAD"] = _run_rfad(
                rfad_reference_dir, rfad_reconstruction_dir, fad_model, output_dir
            )
